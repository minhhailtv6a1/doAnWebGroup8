...
function deleteBill() {
  ...
  var dh_Array = JSON.parse(localStorage.getItem("DS_DH"));
  for (var i = 0; i < bill_Array.length; i++) {
    for (var j = 0; j < dh_Array.length; j++) {
      if (bill_Array[i].id == dh_Array[j].maDH) {
        dh_Array.splice(j, 1);
      }
    }
  }
  ...
  let s = `
   <h2>Thêm Thẻ Tín Dụng</h2>
    <form>
      ...
      <input id="expiredDate" name="expired_date" type="text" placeholder="Ngày hết hạn (MM/YY)" maxlength="5" value="${creditTmp.ngayHetHan}"/>
